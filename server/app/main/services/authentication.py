...
def user_register(data):
    ...
    if data_raw:
        ...
        try:
            new_user = UserModel(
                name=data.get("first_name", "first_name"),
                username=data.get("last_name", "last_name"),
    ...
    if data_raw:
        ...
        if flag:
            ...
            return (
                flag,
                token.decode(),
                {"email": str(data_raw.email), "name": str(data_raw.name)},
            )
...
def otp_generate(data):
    data_raw = UserModel.query.filter(UserModel.mobile == data["mobile"]).first()
    ...
    if data_raw:
        ...
        if flag:
            otp_data = OtpModel.query.filter(OtpModel.mobile == data["mobile"]).first()

            if otp_data:
                db.session.query(OtpModel).filter(
                    OtpModel.mobile == data.get("mobile")
                ).update({OtpModel.otp: otp})
                db.session.commit()
            else:
...
def otp_verify(data):
    data_raw = OtpModel.query.filter(OtpModel.mobile == data["mobile"]).first()
    data_user = UserModel.query.filter(UserModel.mobile == data["mobile"]).first()
    ...
    if data_raw:
        ...
        if flag:
            ...
            return (
                flag,
                token.decode(),
                {"email": str(data_user.email), "name": str(data_user.name)},
            )
...
def oauth_login(data):
    ...
    if data_raw:
        ...
        data_oauth = (
            OauthInfoModel()
            .query.filter(OauthInfoModel.email == data.get("email"))
            .first()
        )
        ...
        if flag_regsiter:
            data_registered = (
                UserModel().query.filter(UserModel.email == data.get("email")).first()
            )
            ...
            return (
                ...
                token.decode(),
                {
                    "email": str(data_registered.email),
                    "name": str(data_registered.name),
                },
            )
        else:
            return False, "", ""
...
def user_logout(token):
    query = BlacklistTokenModel.query.filter(BlacklistTokenModel.token == token).first()
